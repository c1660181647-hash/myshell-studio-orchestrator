...
- `POST /api/studio/actions/resolve-batch` executes multiple safe audit actions, including `verify-ready` coverage checks and dispatch-session `run-target` actions, preserves every manual operator action with instructions, and returns a refreshed audit. Batch `run-target` entries include the dispatch target run result so the Studio UI can continue into client execution requests.
