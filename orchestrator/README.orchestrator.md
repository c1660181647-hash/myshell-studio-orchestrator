...
- `GET /api/studio/delivery-audit` returns machine-readable acceptance status, requirements, de-duplicated operator `actions` from both readiness fixes and handoff gaps, artifacts, and embedded reports for deployment handoff. Dispatch session targets marked `error` are promoted into handoff/audit operator actions. Artifacts include both stable template `endpoint` values and concrete `url` values with project/session/source context filled in. Add `download=1` to receive `myshell-studio-audit-{project_id|current}.json` as a JSON attachment.
- `POST /api/studio/actions/resolve` executes safe audit actions such as targeted `verify-ready`, or returns explicit `manual_required` instructions with concrete `url`/`retryUrl`/`cancelUrl` values for every handoff/audit operator action. Dispatch target actions can include `session_id`, which resolves to the exact dispatch session URL plus `target_id`.
