...
- `GET /api/studio/dispatch-sessions/{session_id}` restores one recoverable dispatch queue, while `POST /api/studio/dispatch-sessions/{session_id}/targets/{target_id}` marks a target `visited`, `completed`, `skipped`, or `error`. The Studio UI exposes Done, Error, and Skip controls for visited queue targets.
